 ...
 * v2.2 http://welldonethings.com/tags/manager
 * =================================================== Copyright 2012 Max
 * Favilli
 * 
 * Licensed under the Mozilla Public License, Version 2.0 You may not use this
    ...
    $.fn.tagsManager = function(options, tagToManipulate) {
        ...
        var setupTypeahead = function() {
            ...
            if (tagManagerOptions.typeaheadSource != null
                    && $.isFunction(tagManagerOptions.typeaheadSource)) {
                obj.typeahead({
        ...
        var spliceTag = function(tagId) {
            ...
            if (tagManagerOptions.maxTags > 0
                    && tlis.length < tagManagerOptions.maxTags) {
                obj.show();
            }
        };
        ...
        var pushTag = function(tag) {
            ...
            if (tagManagerOptions.validator !== undefined) {
                if (tagManagerOptions.validator(tag) !== true)
                    return;
            }
            ...
            if (alreadyInList) {
                ...
                if (tagManagerOptions.AjaxPush != null) {
                    $.post(tagManagerOptions.AjaxPush, {
                                tag : tag
                            });
                }
                ...
                html +=
                        '<span class="myTag'
                                ...
                                + '"><span>'
                                + tag
                                + '&nbsp;&nbsp;</span><a href="#" class="myTagRemover" id="'
                                + newTagRemoveId + '" TagIdToRemove="' + tagId
                                + '" title="Remove">'
                                + tagManagerOptions.tagCloseIcon
                                + '</a></span> ';
        ...
        return this.each(function() {
            ...
            // let's store some instance specific data directly into the DOM
            // object
            var tlis = new Array();
            var tlid = new Array();
            ...
            var html = "";
            html +=
                    "<input name='" + tagManagerOptions.hiddenTagListName
                            + "' type='hidden' value=''/>";
            obj.after(html);
            obj.data("lhiddenTagList", obj.siblings("input[name='"
                            + tagManagerOptions.hiddenTagListName + "']")[0]);
            ...
            obj.change(function(e) {
                        ...
                        if (ao[0] != undefined) {
                            // console.log('change: typeaheadIsVisible is
                            // visible');
                            // when the user click with the mouse on the
                            // typeahead li element we get the change event
                            // fired twice, once when the input field loose
                            // focus and later with the input field value is
                            // replaced with li value
                            var user_input =
                                    $(this).data('typeahead').$menu
                                            .find('.active').attr('data-value');
                            user_input = trimTag(user_input);
                            if (queuedTag == obj.val()
                                ...
                                // typeahead value pushed:' + queuedTag);
                            }
                        } else {
            ...
            if (1 == 1 || !tagManagerOptions.typeahead) {
                obj.on("blur", function(e) {
                            ...
                            if (tagManagerOptions.typeahead) {
                                var ao = $(".typeahead:visible");
                                if (ao[0] != undefined) {
                                    // console.log('blur: typeaheadIsVisible is
                                    // visible');
                                    push = false;
                                } else {
                                    // console.log('blur: typeaheadIsVisible is
