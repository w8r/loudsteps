...
/*!
    ...
    * MIT License
    */
(function(name,context,definition){typeof module!="undefined"&&module.exports?module.exports=definition():typeof context["define"]=="function"&&context.define.amd?define(definition):context[name]=definition()})("qwery",this,function(){function cache(){this.c={}}function classRegex(c){return classCache.g(c)||classCache.s(c,"(^|\\s+)"+c+"(\\s+|$)",1)}function each(a,fn){var i=0,l=a.length;for(;i<l;i++)fn(a[i])}function flatten(ar){for(var r=[],i=0,l=ar.length;i<l;++i)arrayLike(ar[i])?r=r.concat(ar[i]):r[r.length]=ar[i];return r}function arrayify(ar){var i=0,l=ar.length,r=[];for(;i<l;i++)r[i]=ar[i];return r}function previous(n){while(n=n.previousSibling)if(n[nodeType]==1)break;return n}function q(query){return query.match(chunker)}function interpret(whole,tag,idsAndClasses,wholeAttribute,attribute,qualifier,value,wholePseudo,pseudo,wholePseudoVal,pseudoVal){var i,m,k,o,classes;if(this[nodeType]!==1)return!1;if(tag&&tag!=="*"&&this[tagName]&&this[tagName].toLowerCase()!==tag)return!1;if(idsAndClasses&&(m=idsAndClasses.match(id))&&m[1]!==this.id)return!1;if(idsAndClasses&&(classes=idsAndClasses.match(clas)))for(i=classes.length;i--;)if(!classRegex(classes[i].slice(1)).test(this.className))return!1;if(pseudo&&qwery.pseudos[pseudo]&&!qwery.pseudos[pseudo](this,pseudoVal))return!1;if(wholeAttribute&&!value){o=this.attributes;for(k in o)if(Object.prototype.hasOwnProperty.call(o,k)&&(o[k].name||k)==attribute)return this}return wholeAttribute&&!checkAttr(qualifier,getAttr(this,attribute)||"",value)?!1:this}function clean(s){return cleanCache.g(s)||cleanCache.s(s,s.replace(specialChars,"\\$1"))}function checkAttr(qualify,actual,val){switch(qualify){case"=":return actual==val;case"^=":return actual.match(attrCache.g("^="+val)||attrCache.s("^="+val,"^"+clean(val),1));case"$=":return actual.match(attrCache.g("$="+val)||attrCache.s("$="+val,clean(val)+"$",1));case"*=":return actual.match(attrCache.g(val)||attrCache.s(val,clean(val),1));case"~=":return actual.match(attrCache.g("~="+val)||attrCache.s("~="+val,"(?:^|\\s+)"+clean(val)+"(?:\\s+|$)",1));case"|=":return actual.match(attrCache.g("|="+val)||attrCache.s("|="+val,"^"+clean(val)+"(-|$)",1))}return 0}function _qwery(selector,_root){var r=[],ret=[],i,l,m,token,tag,els,intr,item,root=_root,tokens=tokenCache.g(selector)||tokenCache.s(selector,selector.split(tokenizr)),dividedTokens=selector.match(dividers);if(!tokens.length)return r;token=(tokens=tokens.slice(0)).pop();tokens.length&&(m=tokens[tokens.length-1].match(idOnly))&&(root=byId(_root,m[1]));if(!root)return r;intr=q(token);els=root!==_root&&root[nodeType]!==9&&dividedTokens&&/^[+~]$/.test(dividedTokens[dividedTokens.length-1])?function(r){while(root=root.nextSibling)root[nodeType]==1&&(intr[1]?intr[1]==root[tagName].toLowerCase():1)&&(r[r.length]=root);return r}([]):root[byTag](intr[1]||"*");for(i=0,l=els.length;i<l;i++)if(item=interpret.apply(els[i],intr))r[r.length]=item;if(!tokens.length)return r;each(r,function(e){ancestorMatch(e,tokens,dividedTokens)&&(ret[ret.length]=e)});return ret}function is(el,selector,root){if(isNode(selector))return el==selector;if(arrayLike(selector))return!!~flatten(selector).indexOf(el);var selectors=selector.split(","),tokens,dividedTokens;while(selector=selectors.pop()){tokens=tokenCache.g(selector)||tokenCache.s(selector,selector.split(tokenizr));dividedTokens=selector.match(dividers);tokens=tokens.slice(0);if(interpret.apply(el,q(tokens.pop()))&&(!tokens.length||ancestorMatch(el,tokens,dividedTokens,root)))return!0}return!1}function ancestorMatch(el,tokens,dividedTokens,root){function crawl(e,i,p){while(p=walker[dividedTokens[i]](p,e))if(isNode(p)&&interpret.apply(p,q(tokens[i]))){if(!i)return p;if(cand=crawl(p,i-1,p))return cand}}var cand;return(cand=crawl(el,tokens.length-1,el))&&(!root||isAncestor(cand,root))}function isNode(el,t){return el&&typeof el=="object"&&(t=el[nodeType])&&(t==1||t==9)}function uniq(ar){var a=[],i,j;e:for(i=0;i<ar.length;++i){for(j=0;j<a.length;++j)if(a[j]==ar[i])continue e;a[a.length]=ar[i]}return a}function arrayLike(o){return typeof o=="object"&&isFinite(o.length)}function normalizeRoot(root){return root?typeof root=="string"?qwery(root)[0]:!root[nodeType]&&arrayLike(root)?root[0]:root:doc}function byId(root,id,el){return root[nodeType]===9?root.getElementById(id):root.ownerDocument&&((el=root.ownerDocument.getElementById(id))&&isAncestor(el,root)&&el||!isAncestor(root,root.ownerDocument)&&select('[id="'+id+'"]',root)[0])}function qwery(selector,_root){var m,el,root=normalizeRoot(_root);if(!root||!selector)return[];if(selector===window||isNode(selector))return!_root||selector!==window&&isNode(root)&&isAncestor(selector,root)?[selector]:[];if(selector&&arrayLike(selector))return flatten(selector);if(m=selector.match(easy)){if(m[1])return(el=byId(root,m[1]))?[el]:[];if(m[2])return arrayify(root[byTag](m[2]));if(hasByClass&&m[3])return arrayify(root[byClass](m[3]))}return select(selector,root)}function collectSelector(root,collector){return function(s){var oid,nid;if(splittable.test(s)){if(root[nodeType]!==9){(nid=oid=root.getAttribute("id"))||root.setAttribute("id",nid="__qwerymeupscotty");s='[id="'+nid+'"]'+s;collector(root.parentNode||root,s,!0);oid||root.removeAttribute("id")}return}s.length&&collector(root,s,!1)}}var doc=document,html=doc.documentElement,byClass="getElementsByClassName",byTag="getElementsByTagName",qSA="querySelectorAll",useNativeQSA="useNativeQSA",tagName="tagName",nodeType="nodeType",select,id=/#([\w\-]+)/,clas=/\.[\w\-]+/g,idOnly=/^#([\w\-]+)$/,classOnly=/^\.([\w\-]+)$/,tagOnly=/^([\w\-]+)$/,tagAndOrClass=/^([\w]+)?\.([\w\-]+)$/,splittable=/(^|,)\s*[>~+]/,normalizr=/^\s+|\s*([,\s\+\~>]|$)\s*/g,splitters=/[\s\>\+\~]/,splittersMore=/(?![\s\w\-\/\?\&\=\:\.\(\)\!,@#%<>\{\}\$\*\^'"]*\]|[\s\w\+\-]*\))/,specialChars=/([.*+?\^=!:${}()|\[\]\/\\])/g,simple=/^(\*|[a-z0-9]+)?(?:([\.\#]+[\w\-\.#]+)?)/,attr=/\[([\w\-]+)(?:([\|\^\$\*\~]?\=)['"]?([ \w\-\/\?\&\=\:\.\(\)\!,@#%<>\{\}\$\*\^]+)["']?)?\]/,pseudo=/:([\w\-]+)(\(['"]?([^()]+)['"]?\))?/,easy=new RegExp(idOnly.source+"|"+tagOnly.source+"|"+classOnly.source),dividers=new RegExp("("+splitters.source+")"+splittersMore.source,"g"),tokenizr=new RegExp(splitters.source+splittersMore.source),chunker=new RegExp(simple.source+"("+attr.source+")?"+"("+pseudo.source+")?"),walker={" ":function(node){return node&&node!==html&&node.parentNode},">":function(node,contestant){return node&&node.parentNode==contestant.parentNode&&node.parentNode},"~":function(node){return node&&node.previousSibling},"+":function(node,contestant,p1,p2){return node?(p1=previous(node))&&(p2=previous(contestant))&&p1==p2&&p1:!1}};cache.prototype={g:function(k){return this.c[k]||undefined},s:function(k,v,r){v=r?new RegExp(v):v;return this.c[k]=v}};var classCache=new cache,cleanCache=new cache,attrCache=new cache,tokenCache=new cache,isAncestor="compareDocumentPosition"in html?function(element,container){return(container.compareDocumentPosition(element)&16)==16}:"contains"in html?function(element,container){container=container[nodeType]===9||container==window?html:container;return container!==element&&container.contains(element)}:function(element,container){while(element=element.parentNode)if(element===container)return 1;return 0},getAttr=function(){var e=doc.createElement("p");return(e.innerHTML='<a href="#x">x</a>')&&e.firstChild.getAttribute("href")!="#x"?function(e,a){return a==="class"?e.className:a==="href"||a==="src"?e.getAttribute(a,2):e.getAttribute(a)}:function(e,a){return e.getAttribute(a)}}(),hasByClass=!!doc[byClass],hasQSA=doc.querySelector&&doc[qSA],selectQSA=function(selector,root){var result=[],ss,e;try{if(root[nodeType]===9||!splittable.test(selector))return arrayify(root[qSA](selector));each(ss=selector.split(","),collectSelector(root,function(ctx,s){e=ctx[qSA](s);e.length==1?result[result.length]=e.item(0):e.length&&(result=result.concat(arrayify(e)))}));return ss.length>1&&result.length>1?uniq(result):result}catch(ex){}return selectNonNative(selector,root)},selectNonNative=function(selector,root){var result=[],items,m,i,l,r,ss;selector=selector.replace(normalizr,"$1");if(m=selector.match(tagAndOrClass)){r=classRegex(m[2]);items=root[byTag](m[1]||"*");for(i=0,l=items.length;i<l;i++)r.test(items[i].className)&&(result[result.length]=items[i]);return result}each(ss=selector.split(","),collectSelector(root,function(ctx,s,rewrite){r=_qwery(s,ctx);for(i=0,l=r.length;i<l;i++)if(ctx[nodeType]===9||rewrite||isAncestor(r[i],root))result[result.length]=r[i]}));return ss.length>1&&result.length>1?uniq(result):result},configure=function(options){typeof options[useNativeQSA]!="undefined"&&(select=options[useNativeQSA]?hasQSA?selectQSA:selectNonNative:selectNonNative)};configure({useNativeQSA:!0});qwery.configure=configure;qwery.uniq=uniq;qwery.is=is;qwery.pseudos={};return qwery});provide("qwery",module.exports);(function($){var q=function(){var r;try{r=require("qwery")}catch(ex){r=require("qwery-mobile")}finally{return r}}();$.pseudos=q.pseudos;$._select=function(s,r){return($._select=function(){var b;if(typeof $.create=="function")return function(s,r){return/^\s*</.test(s)?$.create(s,r):q(s,r)};try{b=require("bonzo");return function(s,r){return/^\s*</.test(s)?b.create(s,r):q(s,r)}}catch(e){}return q}())(s,r)};$.ender({find:function(s){var r=[],i,l,j,k,els;for(i=0,l=this.length;i<l;i++){els=q(s,this[i]);for(j=0,k=els.length;j<k;j++)r.push(els[j])}return $(q.uniq(r))},and:function(s){var plus=$(s);for(var i=this.length,j=0,l=this.length+plus.length;i<l;i++,j++)this[i]=plus[j];this.length+=plus.length;return this},is:function(s,r){var i,l;for(i=0,l=this.length;i<l;i++)if(q.is(this[i],s,r))return!0;return!1}},!0)})(ender)})();(function(){function keypress(stream){function onData(b){if(stream.listeners("keypress").length>0)emitKey(stream,b);else{stream.removeListener("data",onData);stream.on("newListener",onNewListener)}}function onNewListener(event){if(event=="keypress"){stream.on("data",onData);stream.removeListener("newListener",onNewListener)}}if(isEmittingKeypress(stream))return;stream._emitKeypress=!0;stream.listeners("keypress").length>0?stream.on("data",onData):stream.on("newListener",onNewListener)}function isEmittingKeypress(stream){var rtn=stream._emitKeypress;rtn||stream.listeners("data").forEach(function(l){if(l.name=="onData"&&/emitKey/.test(l.toString())){rtn=!0;stream._emitKeypress=!0}});rtn||stream.listeners("newListener").forEach(function(l){if(l.name=="onNewListener"&&/keypress/.test(l.toString())){rtn=!0;stream._emitKeypress=!0}});return rtn}function emitKey(stream,s){var ch,key={name:undefined,ctrl:!1,meta:!1,shift:!1},parts;if(Buffer.isBuffer(s))if(s[0]>127&&s[1]===undefined){s[0]-=128;s=""+s.toString(stream.encoding||"utf-8")}else s=s.toString(stream.encoding||"utf-8");key.sequence=s;if(s==="\r"||s==="\n")key.name="enter";else if(s==="	")key.name="tab";else if(s==="\b"||s===""||s===""||s==="\b"){key.name="backspace";key.meta=s.charAt(0)===""}else if(s===""||s===""){key.name="escape";key.meta=s.length===2}else if(s===" "||s===" "){key.name="space";key.meta=s.length===2}else if(s<=""){key.name=String.fromCharCode(s.charCodeAt(0)+"a".charCodeAt(0)-1);key.ctrl=!0}else if(s.length===1&&s>="a"&&s<="z")key.name=s;else if(s.length===1&&s>="A"&&s<="Z"){key.name=s.toLowerCase();key.shift=!0}else if(parts=metaKeyCodeRe.exec(s)){key.name=parts[1].toLowerCase();key.meta=!0;key.shift=/^[A-Z]$/.test(parts[1])}else if(parts=functionKeyCodeRe.exec(s)){var code=(parts[1]||"")+(parts[2]||"")+(parts[4]||"")+(parts[6]||""),modifier=(parts[3]||parts[5]||1)-1;key.ctrl=!!(modifier&4);key.meta=!!(modifier&10);key.shift=!!(modifier&1);key.code=code;switch(code){case"OP":key.name="f1";break;case"OQ":key.name="f2";break;case"OR":key.name="f3";break;case"OS":key.name="f4";break;case"[11~":key.name="f1";break;case"[12~":key.name="f2";break;case"[13~":key.name="f3";break;case"[14~":key.name="f4";break;case"[[A":key.name="f1";break;case"[[B":key.name="f2";break;case"[[C":key.name="f3";break;case"[[D":key.name="f4";break;case"[[E":key.name="f5";break;case"[15~":key.name="f5";break;case"[17~":key.name="f6";break;case"[18~":key.name="f7";break;case"[19~":key.name="f8";break;case"[20~":key.name="f9";break;case"[21~":key.name="f10";break;case"[23~":key.name="f11";break;case"[24~":key.name="f12";break;case"[A":key.name="up";break;case"[B":key.name="down";break;case"[C":key.name="right";break;case"[D":key.name="left";break;case"[E":key.name="clear";break;case"[F":key.name="end";break;case"[H":key.name="home";break;case"OA":key.name="up";break;case"OB":key.name="down";break;case"OC":key.name="right";break;case"OD":key.name="left";break;case"OE":key.name="clear";break;case"OF":key.name="end";break;case"OH":key.name="home";break;case"[1~":key.name="home";break;case"[2~":key.name="insert";break;case"[3~":key.name="delete";break;case"[4~":key.name="end";break;case"[5~":key.name="pageup";break;case"[6~":key.name="pagedown";break;case"[[5~":key.name="pageup";break;case"[[6~":key.name="pagedown";break;case"[7~":key.name="home";break;case"[8~":key.name="end";break;case"[a":key.name="up";key.shift=!0;break;case"[b":key.name="down";key.shift=!0;break;case"[c":key.name="right";key.shift=!0;break;case"[d":key.name="left";key.shift=!0;break;case"[e":key.name="clear";key.shift=!0;break;case"[2$":key.name="insert";key.shift=!0;break;case"[3$":key.name="delete";key.shift=!0;break;case"[5$":key.name="pageup";key.shift=!0;break;case"[6$":key.name="pagedown";key.shift=!0;break;case"[7$":key.name="home";key.shift=!0;break;case"[8$":key.name="end";key.shift=!0;break;case"Oa":key.name="up";key.ctrl=!0;break;case"Ob":key.name="down";key.ctrl=!0;break;case"Oc":key.name="right";key.ctrl=!0;break;case"Od":key.name="left";key.ctrl=!0;break;case"Oe":key.name="clear";key.ctrl=!0;break;case"[2^":key.name="insert";key.ctrl=!0;break;case"[3^":key.name="delete";key.ctrl=!0;break;case"[5^":key.name="pageup";key.ctrl=!0;break;case"[6^":key.name="pagedown";key.ctrl=!0;break;case"[7^":key.name="home";key.ctrl=!0;break;case"[8^":key.name="end";key.ctrl=!0;break;case"[Z":key.name="tab";key.shift=!0;break;default:key.name="undefined"}}else if(s.length>1&&s[0]!==""){Array.prototype.forEach.call(s,function(c){emitKey(stream,c)});return}if(key.code=="[M"){key.name="mouse";var s=key.sequence,b=s.charCodeAt(3);key.x=s.charCodeAt(4)-32;key.y=s.charCodeAt(5)-32;key.scroll=0;key.ctrl=!!(16&b);key.meta=!!(8&b);key.shift=!!(4&b);key.release=(3&b)===3;64&b&&(key.scroll=1&b?1:-1);!key.release&&!key.scroll&&(key.button=b&3)}key.name===undefined&&(key=undefined);s.length===1&&(ch=s);key&&key.name=="mouse"?stream.emit("mousepress",key):(key||ch)&&stream.emit("keypress",ch,key)}var module={exports:{}},exports=module.exports,exports=module.exports=keypress;exports.enableMouse=function(stream){stream.write("[?1000h")};exports.disableMouse=function(stream){stream.write("[?1000l")};var metaKeyCodeRe=/^(?:\x1b)([a-zA-Z0-9])$/,functionKeyCodeRe=/^(?:\x1b+)(O|N|\[|\[\[)(?:(\d+)(?:;(\d+))?([~^$])|(?:1;)?(\d+)?([a-zA-Z]))/;provide("keypress",module.exports);$.ender(module.exports)})();(function(){function Option(flags,description){this.flags=flags;this.required=~flags.indexOf("<");this.optional=~flags.indexOf("[");this.bool=!~flags.indexOf("-no-");flags=flags.split(/[ ,|]+/);flags.length>1&&!/^[[<]/.test(flags[1])&&(this.short=flags.shift());this.long=flags.shift();this.description=description||""}function Command(name){this.commands=[];this.options=[];this._args=[];this._name=name}function camelcase(flag){return flag.split("-").reduce(function(str,word){return str+word[0].toUpperCase()+word.slice(1)})}function parseBool(str){return/^y|yes|ok|true$/i.test(str)}function pad(str,width){var len=Math.max(0,width-str.length);return str+Array(len+1).join(" ")}function outputHelpIfNecessary(cmd,options){options=options||[];for(var i=0;i<options.length;i++)if(options[i]=="--help"||options[i]=="-h"){cmd.outputHelp();process.exit(0)}}var module={exports:{}},exports=module.exports;
/*!
   * commander
   * Copyright(c) 2011 TJ Holowaychuk <tj@vision-media.ca>
   * MIT Licensed
   */
;var EventEmitter=require("events").EventEmitter,spawn=require("child_process").spawn,keypress=require("keypress"),fs=require("fs"),exists=fs.existsSync,path=require("path"),tty=require("tty"),dirname=path.dirname,basename=path.basename;exports=module.exports=new Command;exports.Command=Command;exports.Option=Option;Option.prototype.name=function(){return this.long.replace("--","").replace("no-","")};Option.prototype.is=function(arg){return arg==this.short||arg==this.long};Command.prototype.__proto__=EventEmitter.prototype;Command.prototype.command=function(name,desc){var args=name.split(/ +/),cmd=new Command(args.shift());desc&&cmd.description(desc);desc&&(this.executables=!0);this.commands.push(cmd);cmd.parseExpectedArgs(args);cmd.parent=this;return desc?this:cmd};Command.prototype.addImplicitHelpCommand=function(){this.command("help [cmd]","display help for [cmd]")};Command.prototype.parseExpectedArgs=function(args){if(!args.length)return;var self=this;args.forEach(function(arg){switch(arg[0]){case"<":self._args.push({required:!0,name:arg.slice(1,-1)});break;case"[":self._args.push({required:!1,name:arg.slice(1,-1)})}});return this};Command.prototype.action=function(fn){var self=this;this.parent.on(this._name,function(args,unknown){unknown=unknown||[];var parsed=self.parseOptions(unknown);outputHelpIfNecessary(self,parsed.unknown);parsed.unknown.length>0&&self.unknownOption(parsed.unknown[0]);parsed.args.length&&(args=parsed.args.concat(args));self._args.forEach(function(arg,i){arg.required&&null==args[i]&&self.missingArgument(arg.name)});self._args.length?args[self._args.length]=self:args.push(self);fn.apply(this,args)});return this};Command.prototype.option=function(flags,description,fn,defaultValue){var self=this,option=new Option(flags,description),oname=option.name(),name=camelcase(oname);"function"!=typeof fn&&(defaultValue=fn,fn=null);if(0==option.bool||option.optional||option.required){0==option.bool&&(defaultValue=!0);undefined!==defaultValue&&(self[name]=defaultValue)}this.options.push(option);this.on(oname,function(val){null!=val&&fn&&(val=fn(val));"boolean"==typeof self[name]||"undefined"==typeof self[name]?null==val?self[name]=option.bool?defaultValue||!0:!1:self[name]=val:null!==val&&(self[name]=val)});return this};Command.prototype.parse=function(argv){this.executables&&this.addImplicitHelpCommand();this.rawArgs=argv;this._name=this._name||basename(argv[1]);var parsed=this.parseOptions(this.normalize(argv.slice(2))),args=this.args=parsed.args;return this.executables?this.executeSubCommand(argv,args,parsed.unknown):this.parseArgs(this.args,parsed.unknown)};Command.prototype.executeSubCommand=function(argv,args,unknown){args=args.concat(unknown);args.length||this.help();"help"==args[0]&&1==args.length&&this.help();if("help"==args[0]){args[0]=args[1];args[1]="--help"}var dir=dirname(argv[1]),bin=basename(argv[1])+"-"+args[0],local=path.join(dir,bin);exists(local)&&(bin=local);args=args.slice(1);var proc=spawn(bin,args,{stdio:"inherit",customFds:[0,1,2]});proc.on("exit",function(code){code==127&&console.error("\n  %s(1) does not exist\n",bin)})};Command.prototype.normalize=function(args){var ret=[],arg,index;for(var i=0,len=args.length;i<len;++i){arg=args[i];arg.length>1&&"-"==arg[0]&&"-"!=arg[1]?arg.slice(1).split("").forEach(function(c){ret.push("-"+c)}):/^--/.test(arg)&&~(index=arg.indexOf("="))?ret.push(arg.slice(0,index),arg.slice(index+1)):ret.push(arg)}return ret};Command.prototype.parseArgs=function(args,unknown){var cmds=this.commands,len=cmds.length,name;if(args.length){name=args[0];this.listeners(name).length?this.emit(args.shift(),args,unknown):this.emit("*",args)}else{outputHelpIfNecessary(this,unknown);unknown.length>0&&this.unknownOption(unknown[0])}return this};Command.prototype.optionFor=function(arg){for(var i=0,len=this.options.length;i<len;++i)if(this.options[i].is(arg))return this.options[i]};Command.prototype.parseOptions=function(argv){var args=[],len=argv.length,literal,option,arg,unknownOptions=[];for(var i=0;i<len;++i){arg=argv[i];if("--"==arg){literal=!0;continue}if(literal){args.push(arg);continue}option=this.optionFor(arg);if(option){if(option.required){arg=argv[++i];if(null==arg)return this.optionMissingArgument(option);if("-"==arg[0])return this.optionMissingArgument(option,arg);this.emit(option.name(),arg)}else if(option.optional){arg=argv[i+1];null==arg||"-"==arg[0]?arg=null:++i;this.emit(option.name(),arg)}else this.emit(option.name());continue}if(arg.length>1&&"-"==arg[0]){unknownOptions.push(arg);argv[i+1]&&"-"!=argv[i+1][0]&&unknownOptions.push(argv[++i]);continue}args.push(arg)}return{args:args,unknown:unknownOptions}};Command.prototype.missingArgument=function(name){console.error();console.error("  error: missing required argument `%s'",name);console.error();process.exit(1)};Command.prototype.optionMissingArgument=function(option,flag){console.error();flag?console.error("  error: option `%s' argument missing, got `%s'",option.flags,flag):console.error("  error: option `%s' argument missing",option.flags);console.error();process.exit(1)};Command.prototype.unknownOption=function(flag){console.error();console.error("  error: unknown option `%s'",flag);console.error();process.exit(1)};Command.prototype.version=function(str,flags){if(0==arguments.length)return this._version;this._version=str;flags=flags||"-V, --version";this.option(flags,"output the version number");this.on("version",function(){console.log(str);process.exit(0)});return this};Command.prototype.description=function(str){if(0==arguments.length)return this._description;this._description=str;return this};Command.prototype.usage=function(str){var args=this._args.map(function(arg){return arg.required?"<"+arg.name+">":"["+arg.name+"]"}),usage="[options"+(this.commands.length?"] [command":"")+"]"+(this._args.length?" "+args:"");if(0==arguments.length)return this._usage||usage;this._usage=str;return this};Command.prototype.largestOptionLength=function(){return this.options.reduce(function(max,option){return Math.max(max,option.flags.length)},0)};Command.prototype.optionHelp=function(){var width=this.largestOptionLength();return[pad("-h, --help",width)+"  "+"output usage information"].concat(this.options.map(function(option){return pad(option.flags,width)+"  "+option.description})).join("\n")};Command.prototype.commandHelp=function(){return this.commands.length?["","  Commands:","",this.commands.map(function(cmd){var args=cmd._args.map(function(arg){return arg.required?"<"+arg.name+">":"["+arg.name+"]"}).join(" ");return pad(cmd._name+(cmd.options.length?" [options]":"")+" "+args,22)+(cmd.description()?" "+cmd.description():"")}).join("\n").replace(/^/gm,"    "),""].join("\n"):""};Command.prototype.helpInformation=function(){return["","  Usage: "+this._name+" "+this.usage(),""+this.commandHelp(),"  Options:","",""+this.optionHelp().replace(/^/gm,"    "),"",""].join("\n")};Command.prototype.promptForNumber=function(str,fn){var self=this;this.promptSingleLine(str,function e(val){val=Number(val);if(isNaN(val))return self.promptSingleLine(str+"(must be a number) ",parseNumber);fn(val)})};Command.prototype.promptForDate=function(str,fn){var self=this;this.promptSingleLine(str,function e(val){val=new Date(val);if(isNaN(val.getTime()))return self.promptSingleLine(str+"(must be a date) ",parseDate);fn(val)})};Command.prototype.promptSingleLine=function(str,fn){if("function"==typeof arguments[2])return this["promptFor"+(fn.name||fn)](str,arguments[2]);process.stdout.write(str);process.stdin.setEncoding("utf8");process.stdin.once("data",function(val){fn(val.trim())}).resume()};Command.prototype.promptMultiLine=function(str,fn){var buf=[];console.log(str);process.stdin.setEncoding("utf8");process.stdin.on("data",function(val){if("\n"==val||"\r\n"==val){process.stdin.removeAllListeners("data");fn(buf.join("\n"))}else buf.push(val.trimRight())}).resume()};Command.prototype.prompt=function(str,fn){var self=this;if("string"==typeof str){if(/ $/.test(str))return this.promptSingleLine.apply(this,arguments);this.promptMultiLine(str,fn)}else{var keys=Object.keys(str),obj={};function next(){var key=keys.shift(),label=str[key];if(!key)return fn(obj);self.prompt(label,function(val){obj[key]=val;next()})}next()}};Command.prototype.password=function(str,mask,fn){function setRawMode(mode){process.stdin.setRawMode?process.stdin.setRawMode(mode):tty.setRawMode(mode)}var self=this,buf="";if("function"==typeof mask){fn=mask;mask=""}keypress(process.stdin);setRawMode(!0);process.stdout.write(str);process.stdin.on("keypress",function(c,key){if(key&&"enter"==key.name){console.log();process.stdin.pause();process.stdin.removeAllListeners("keypress");setRawMode(!1);if(!buf.trim().length)return self.password(str,mask,fn);fn(buf);return}if(key&&key.ctrl&&"c"==key.name){console.log("%s",buf);process.exit()}process.stdout.write(mask);buf+=c}).resume()};Command.prototype.confirm=function(str,fn,verbose){var self=this;this.prompt(str,function(ok){if(!ok.trim()){verbose||(str+="(yes or no) ");return self.confirm(str,fn,!0)}fn(parseBool(ok))})};Command.prototype.choose=function(list,index,fn){function again(){self.prompt("  : ",function(val){val=parseInt(val,10)-1;hasDefault&&isNaN(val)&&(val=index);null==list[val]?again():fn(val,list[val])})}var self=this,hasDefault="number"==typeof index;if(!hasDefault){fn=index;index=null}list.forEach(function(item,i){hasDefault&&i==index?console.log("* %d) %s",i+1,item):console.log("  %d) %s",i+1,item)});again()};Command.prototype.outputHelp=function(){process.stdout.write(this.helpInformation());this.emit("--help")};Command.prototype.help=function(){this.outputHelp();process.exit()};provide("commander",module.exports);$.ender(module.exports)})()